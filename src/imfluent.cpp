...
namespace ImFluentInternal
{
    ...
    {
        const ImFluentStyle & style = ImFluent::GetStyle();
        const float outer = ImFluent::FluentDpx( style.FocusStrokeThicknessOuter );
        const float inner = ImFluent::FluentDpx( style.FocusStrokeThicknessInner );
    ...
    void DrawElevationShadow( ImDrawList * dl, const ImRect & bb, float rounding, int layers )
    {
        ...
        for ( int i = 0; i < layers; ++i )
        {
            ...
            const float pad = ImFluent::FluentDpx( ImFluent::GetStyle().SpacingXSmall * k );
    ...
    {
        const ImFluentStyle & style = ImFluent::GetStyle();
        const float thickness = ImFluent::FluentDpx( style.SelectionIndicatorThickness );
        const float inset     = ImFluent::FluentDpx( style.SelectionIndicatorInset );
        const float corner    = ImFluent::FluentDpx( style.SpacingXSmall );
...
namespace ImFluent
{
    ...
    bool RadioButtons( const char * label, int * v, const char * const items[], int items_count, int max_columns )
    {
    ...
    bool RangeSlider( const char * label, float * v_min, float * v_max, float v_lo, float v_hi, const char * format )
    {
        ...
        dl->AddRectFilled( ImVec2( track_left, track_y - track_h * 0.5f ),
                           ...
                           GetColorU32( ImFluentCol_ControlStrongFillDefault ), track_h * 0.5f );
    ...
    bool NumberBox( const char * label, double * v, double step, double step_fast, const char * format, ImGuiInputTextFlags flags )
    {
        ...
        for ( int i = 0; i < 2; ++i )
        {
            ...
            DrawChevron( dl, c, b.dir, GetColorU32( ImFluentCol_TextSecondary ), FluentDpx( style.ChevronGlyphSize - 1.f ) );
        }
    ...
    bool BeginSettingsCard( const char * id, const char * header, const char * description, const char * glyph )
    {
        const ImFluentStyle & style = GetStyle();
        ...
        if ( description )
            dl->AddText( ImVec2( text_x, text_y + font_h + gap_y ),
                         GetColorU32( ImFluentCol_TextSecondary ), description );
    ...
    void BeginStackPanelHorizontal( float spacing )
    {
        const ImFluentStyle & style = GetStyle();
    ...
    void BeginStackPanelVertical( float spacing )
    {
        const ImFluentStyle & style = GetStyle();
        if ( spacing < 0.f ) spacing = FluentDpx( style.SpacingMedium );
        ImGui::BeginGroup();
    ...
    void BeginWrapPanel( float h_spacing, float v_spacing )
    {
        const ImFluentStyle & style = GetStyle();
    ...
    bool WrapPanelNextItem( float item_width )
    {
    ...
    bool BeginExpander( const char * label, bool * open )
    {
        ...
        dl->AddLine( a, b, GetColorU32( ImFluentCol_TextPrimary ), FluentDpx( style.StrokeMedium ) );
        dl->AddLine( b, c, GetColorU32( ImFluentCol_TextPrimary ), FluentDpx( style.StrokeMedium ) );
        ( void )ang;

        dl->AddText( ImVec2( bb.Min.x + FluentDpx( style.SpacingXLarge ), cy - ImGui::GetFontSize() * 0.5f ), GetColorU32( ImFluentCol_TextPrimary ), label );
        if ( IsItemFocused( id ) ) DrawFocusRing( dl, bb, r );
    ...
    bool SelectorBarItem( const char * label, bool selected, const char * glyph )
    {
        ...
        if ( selected )
        {
            ...
            dl->AddRectFilled( ImVec2( bx - bar_w * 0.5f, bb.Max.y - FluentDpx( style.SpacingXSmall ) ),
                               ...
                               GetColorU32( ImFluentCol_AccentFillDefault ), FluentDpx( style.StrokeThin ) );
        }
        if ( IsItemFocused( id ) ) DrawFocusRing( dl, bb, FluentDpx( style.ControlCornerRadius ) );
        ImGui::SameLine();
        return pressed;
    ...
    bool BeginNavigationView( const char * id, ImFluentNavViewMode * mode_io )
    {
        ...
        }
        {
            ...
            if ( hov ) dl->AddRectFilled( bb.Min, bb.Max, GetColorU32( ImFluentCol_SubtleFillSecondary ), FluentDpx( style.ControlCornerRadius ) );
            ...
            for ( int i = -1; i <= 1; ++i )
                dl->AddLine( ImVec2( cx - lw * 0.5f, cy + ( float )i * FluentDpx( style.SpacingSmall + 1.f ) ),
                             ...
                             GetColorU32( ImFluentCol_TextPrimary ), FluentDpx( style.StrokeMedium ) );
        }
        return true;
    }
    ...
    bool NavItem( const char * label, bool selected, const char * glyph )
    {
        ...
        if ( selected )
        {
            ...
            dl->AddRectFilled( ImVec2( bx, (bb.Min.y + bb.Max.y - bh) * 0.5f ),
                               ...
                               GetColorU32( ImFluentCol_AccentFillDefault ), FluentDpx( style.SpacingXSmall ) );
        }
    ...
    {
        const ImFluentStyle & style = GetStyle();
        if ( g_NavView.CurrentWidth < ImFluent::FluentDpx( style.AppBarButtonWidth + style.SpacingXXLarge + style.SpacingMedium ) ) return;
    ...
    bool NavBackButton( bool can_go_back )
    {
        ImGuiWindow * w = ImGui::GetCurrentWindow();
        if ( w->SkipItems ) return false;
        const ImFluentStyle & style = GetStyle();
        ...
        {
            const ImU32 fill = held ? GetColorU32( ImFluentCol_SubtleFillTertiary )
                                    : GetColorU32( ImFluentCol_SubtleFillSecondary );
            dl->AddRectFilled( bb.Min, bb.Max, fill, r );
        }

        const ImU32 textCol = can_go_back ? GetColorU32( ImFluentCol_TextPrimary )
    ...
    void NavPaneFooterEnd()
    {
        // Pure marker for symmetry; nothing to pop.
    }

    bool NavSettingsItem( bool selected )
    {
    ...
    bool ListViewItem( const char * label, bool selected, const char * glyph )
    {
        ...
        const float cy = (bb.Min.y + bb.Max.y - ImGui::GetFontSize()) * 0.5f;
        if ( glyph ) dl->AddText( ImVec2( bb.Min.x + FluentDpx( style.StandardIconSize - 2.f ), cy ), GetColorU32( ImFluentCol_TextPrimary ), glyph );
        dl->AddText( ImVec2( text_x, cy ), GetColorU32( ImFluentCol_TextPrimary ), label );
        if ( IsItemFocused( id ) ) DrawFocusRing( dl, bb, r );
        return pressed;
    }
    ...
    static bool TreeNodeImpl( const char * label, bool * p_open, bool * p_checked )
    {
        ...
        DrawChevron( dl, ImVec2( chevron_cx, cy ),
                     ...
                     GetColorU32( ImFluentCol_TextPrimary ), FluentDpx( style.ChevronGlyphSize ) );
    ...
    void TreePop() { ImGui::Unindent( FluentDpx( GetStyle().CheckboxSize ) ); }
    ...
    bool PipsPager( const char * id, int * current_item, int total_pages )
    {
        ImGuiWindow * w = ImGui::GetCurrentWindow();
        if ( w->SkipItems ) return false;
        const ImFluentStyle & style = GetStyle(); ( void )style;
    ...
    namespace
    {
        ...
        static void ApplyPopupAnchor( const char * str_id )
        {
            ...
            if ( !anchor ) return;
            const ImVec2 pos( anchor->Min.x, anchor->Max.y + ImFluent::FluentDpx( ImFluent::GetStyle().SpacingXSmall ) );
    ...
    void EndFlyout()
    {
        ...
        if ( ImGuiWindow * w = ImGui::GetCurrentWindow() )
        {
            ...
            ImFluentInternal::DrawElevationShadow( fg, bb, FluentDpx( style.OverlayCornerRadius ), 4 );
        }
        ImGui::EndPopup();
        ImGui::PopStyleVar( 2 );
        ImGui::PopStyleColor( 2 );
    ...
    bool MenuFlyoutItem( const char * label, const char * shortcut, const char * glyph, bool selected, bool enabled )
    {
        ...
        if ( glyph )
            dl->AddText( ImVec2( bb.Min.x + FluentDpx( style.SpacingLarge ), cy ), enabled ? GetColorU32( ImFluentCol_TextPrimary ) : GetColorU32( ImFluentCol_TextDisabled ), glyph );
        dl->AddText( ImVec2( bb.Min.x + FluentDpx( style.NavItemHeight ), cy ), enabled ? GetColorU32( ImFluentCol_TextPrimary ) : GetColorU32( ImFluentCol_TextDisabled ), label );
        if ( shortcut )
            dl->AddText( ImVec2( bb.Max.x - FluentDpx( style.SpacingLarge ) - ss.x, cy ), GetColorU32( ImFluentCol_TextSecondary ), shortcut );
        return pressed;
    }

    bool ToggleMenuFlyoutItem( const char * label, bool * v, const char * shortcut, bool enabled )
    {
    ...
    bool RadioMenuFlyoutItem( const char * label, int * v, int v_button, const char * shortcut, bool enabled )
    {
    ...
    bool BeginMenuFlyoutSubItem( const char * label, const char * glyph, bool enabled )
    {
        ImGuiWindow * w = ImGui::GetCurrentWindow();
        if ( w->SkipItems ) return false;
        glyph = ConsumePendingGlyph( glyph );
        ...
        if ( glyph )
            dl->AddText( ImVec2( bb.Min.x + FluentDpx( style.SpacingLarge ), cy ),
                         enabled ? GetColorU32( ImFluentCol_TextPrimary ) : GetColorU32( ImFluentCol_TextDisabled ), glyph );
        dl->AddText( ImVec2( bb.Min.x + FluentDpx( style.NavItemHeight ), cy ),
                     enabled ? GetColorU32( ImFluentCol_TextPrimary ) : GetColorU32( ImFluentCol_TextDisabled ), label );
        DrawChevron( dl,
    ...
    int EndContentDialog( const char * primary, const char * secondary, const char * close_text, ImFluentContentDialogButton default_button )
    {
        const ImFluentStyle & style = GetStyle();
        int result = 0;
    ...
    void InfoBar( ImFluentInfoSeverity sev, const char * title, const char * msg, bool * is_open, const char * glyph_override )
    {
        ...
        if ( is_open )
        {
            ...
            if ( hov ) dl->AddRectFilled( close_bb.Min, close_bb.Max, GetColorU32( ImFluentCol_SubtleFillSecondary ), FluentDpx( style.ControlCornerRadius ) );
            const ImVec2 cc( ( close_bb.Min.x + close_bb.Max.x ) * 0.5f, ( close_bb.Min.y + close_bb.Max.y ) * 0.5f );
            const float L = FluentDpx( style.ChevronGlyphSize + 1.f );
            dl->AddLine( ImVec2( cc.x - L, cc.y - L ), ImVec2( cc.x + L, cc.y + L ), GetColorU32( ImFluentCol_TextPrimary ), FluentDpx( style.StrokeMedium ) );
            dl->AddLine( ImVec2( cc.x - L, cc.y + L ), ImVec2( cc.x + L, cc.y - L ), GetColorU32( ImFluentCol_TextPrimary ), FluentDpx( style.StrokeMedium ) );
        }
    }
    ...
    void PersonPicture( const char * display_name, float diameter_dpx, const char * glyph_override )
    {
        ImGuiWindow * w = ImGui::GetCurrentWindow();
        if ( w->SkipItems ) return;
        const float D = FluentDpx( diameter_dpx );
        ...
        if ( glyph_override )
        {
            ...
            dl->AddText( ImVec2( c.x - gs.x * 0.5f, c.y - gs.y * 0.5f ),
                         GetColorU32( ImFluentCol_TextOnAccentPrimary ), glyph_override );
        }
        else if ( display_name && *display_name )
        {
            ...
            dl->AddText( ImVec2( c.x - ts.x * 0.5f, c.y - ts.y * 0.5f ),
                         GetColorU32( ImFluentCol_TextOnAccentPrimary ), initials );
        }
        else
        {
            dl->AddText( ImVec2( c.x - ImGui::CalcTextSize( ImFluentIcon_Contact ).x * 0.5f,
                                 c.y - ImGui::GetFontSize() * 0.5f ),
                         GetColorU32( ImFluentCol_TextOnAccentPrimary ), ImFluentIcon_Contact );
        }
    }
    ...
    bool BeginTitleBar( const char * title, float height )
    {
        const ImFluentStyle & style = GetStyle();
    ...
    bool BeginCommandBar( const char * id, float height )
    {
        const ImFluentStyle & style = GetStyle();
    ...
    void SetNextAppBarLabelPosition( ImFluentAppBarLabelPosition pos )
    {
    ...
    bool AppBarButton( const char * label, const char * glyph, const ImVec2 & size_arg )
    {
        ...
        dl->AddRectFilled( bb.Min, bb.Max, AnimateColorU32( id, fill ), r );
        DrawAppBarContent( dl, bb, label, glyph, GetColorU32( ImFluentCol_TextPrimary ), pos, style );
        if ( IsItemFocused( id ) ) DrawFocusRing( dl, bb, r );
        return pressed;
    }

    bool AppBarToggleButton( const char * label, const char * glyph, bool * v, const ImVec2 & size_arg )
    {
        ImGuiWindow * w = ImGui::GetCurrentWindow();
        if ( w->SkipItems ) return false;
        glyph = ConsumePendingGlyph( glyph );
        ...
        dl->AddRectFilled( bb.Min, bb.Max, AnimateColorU32( id, fill ), r );
        const ImU32 textCol = on ? GetColorU32( ImFluentCol_TextOnAccentPrimary )
                                 : GetColorU32( ImFluentCol_TextPrimary );
