...
namespace ImFluentGalleryApp
{
    ...
    static void Page_Item_HyperlinkButton()
    {
        ...
        {
            static int s = 0;
            if ( HyperlinkButton( "Open the Fluent 2 design system" ) )
                ++s;
    ...
    static void Page_Item_SplitButton()
    {
        ...
        if ( BeginControlExample( "Send with options" ) )
        {
            ...
            bool dd1                         = false;
            if ( SplitButton( send_modes[ picked_mode ], &dd1 ) )
                send_count++;
            if ( dd1 )
                OpenMenuFlyout( "##sb-menu1" );
            if ( BeginMenuFlyout( "##sb-menu1" ) )
            {
                SetNextItemGlyph( ImFluentIcon_Mail );
                if ( MenuFlyoutItem( "Send now" ) )
                    picked_mode = 0;
                SetNextItemGlyph( ImFluentIcon_Clock );
                if ( MenuFlyoutItem( "Send later" ) )
                    picked_mode = 1;
                SetNextItemGlyph( ImFluentIcon_Save );
                if ( MenuFlyoutItem( "Save as draft" ) )
                    picked_mode = 2;
                MenuFlyoutSeparator();
                SetNextItemGlyph( ImFluentIcon_Delete );
                if ( MenuFlyoutItem( "Discard" ) )
                    picked_mode = 3;
                EndMenuFlyout();
            }
        ...
        if ( BeginControlExample( "ToggleSplitButton (paragraph alignment)" ) )
        {
            ...
            if ( dd2 )
                OpenMenuFlyout( "##sb-menu2" );
            if ( BeginMenuFlyout( "##sb-menu2" ) )
            {
                for ( int i = 0; i < IM_ARRAYSIZE( aligns ); ++i )
                    if ( RadioMenuFlyoutItem( aligns[ i ], &align_pick, i ) )
                        align_on = true;
                EndMenuFlyout();
            }
    ...
    static void Page_Item_ProgressBar()
    {
        ...
        if ( BeginControlExample( "State (Running / Paused / Error)" ) )
        {
            ...
            {
                for ( int i = 0; i < 3; ++i )
                    if ( SelectorBarItem( state_names[ i ], state == i ) )
                        state = i;
                EndSelectorBar();
            }
    ...
    static void Page_Item_ProgressRing()
    {
        ...
        {
            ProgressRing( 20.f, -1.f );
            ImGui::SameLine();
            ImGui::Dummy( ImVec2( FluentDpx( 16.f ), 0 ) );
            ImGui::SameLine();
            ProgressRing( 32.f, -1.f );
            ImGui::SameLine();
            ImGui::Dummy( ImVec2( FluentDpx( 16.f ), 0 ) );
            ImGui::SameLine();
            ProgressRing( 48.f, -1.f );
            ImGui::SameLine();
            ImGui::Dummy( ImVec2( FluentDpx( 16.f ), 0 ) );
            ImGui::SameLine();
    ...
    static void Page_Item_TextBox()
    {
        ...
        if ( BeginControlExample( "Validation error (SetNextItemError)" ) )
        {
            ...
            if ( email_invalid )
                SetNextItemError( "Enter a valid email address (must contain '@')." );
    ...
    static void Page_Item_AutoSuggestBox()
    {
        ...
        {
            static const char * items[] = { "Apple Pie", "Apricot Jam", "Banana Bread", "Blueberry Muffin", "Carrot Cake", "Cherry Tart", "Chocolate Chip Cookie", "Cinnamon Roll", "Coconut Macaroon", "Cranberry Scone", "Date Bar", "Elderflower Cordial", "Fig Newton", "Ginger Snap", "Grapefruit Sorbet", "Honey Cake", "Kiwi Pavlova", "Lemon Meringue", "Mango Mousse", "Peach Cobbler",
                "Pumpkin Spice Latte", "Raspberry Trifle", "Strawberry Shortcake", "Vanilla Custard", "Walnut Brownie" };
            static char buf[ 64 ]       = "";
            static int selected         = -1;
            ...
            struct Scorer
            {
                ...
                {
                    if ( !filter || !*filter )
                        return 1; // show all in original order when empty
                    ...
                    if ( hit == item )
                        score += 200; // prefix
                    else if ( hit > item && ( hit[ -1 ] == ' ' || hit[ -1 ] == '-' ) )
                        score += 100;               // word-start
    ...
    static void Page_Item_ComboBox()
    {
        ...
        {
            static int tz_sel              = 0;
            static const char * tz_items[] = { "(UTC-08:00) Pacific Time", "(UTC-05:00) Eastern Time", "(UTC+00:00) UTC", "(UTC+01:00) Central Europe", "(UTC+09:00) Tokyo" };
            ImGui::PushItemWidth( 280.f );
    ...
    static void Page_Item_ListView()
    {
        ...
        if ( BeginControlExample( "List" ) )
        {
            ...
            for ( int i = 0; i < 4; ++i )
            {
                if ( ListViewItem( names[ i ], s == i, glyphs[ i ] ) )
                    s = i;
            }
        }
        EndControlExample();
    ...
    static void Page_Item_TreeView()
    {
        ...
        if ( BeginControlExample( "Multi-select (checkbox variant)" ) )
        {
            ...
            ControlExampleOutput( "Checked: %s%s%s", ms_chk[ 0 ] ? "Inbox " : "", ms_chk[ 1 ] ? "Welcome " : "", ms_chk[ 2 ] ? "Project " : "" );
        }
        EndControlExample();
    }
    ...
    static void Page_Item_GridView()
    {
        ...
        if ( BeginControlExample( "Tiles" ) )
        {
            ...
            for ( int i = 0; i < 8; ++i )
            {
                ...
                ( void )buf_end;
                if ( GridViewItem( buf, s == i, ImVec2( 140.f, 100.f ) ) )
                    s = i;
                if ( ( i % 4 ) != 3 )
                    ImGui::SameLine();
            }
    ...
    static void Page_Item_PagerControl()
    {
        ...
        {
            static int page = 2;
            PagerControl( "##pc-fl", &page, 20, ImFluentPagerDisplayMode_ButtonPanel, ImFluentPagerButtonVisibility_Visible, ImFluentPagerButtonVisibility_Visible );
        ...
        {
            static int page = 0;
            PagerControl( "##pc-hide", &page, 10, ImFluentPagerDisplayMode_ButtonPanel, ImFluentPagerButtonVisibility_HiddenOnEdge, ImFluentPagerButtonVisibility_HiddenOnEdge );
    ...
    static void Page_Item_BreadcrumbBar()
    {
        ...
        if ( BeginControlExample( "Breadcrumb" ) )
        {
            ...
            if ( clicked >= 0 )
                ControlExampleOutput( "Clicked index: %d (%s)", clicked, items[ clicked ] );
        }
        EndControlExample();
    }
    ...
    static void Page_Item_Expander()
    {
        ...
        if ( BeginControlExample( "Up direction (body opens above)" ) )
        {
            ...
            if ( BeginExpander( "Notification preferences", &s_open, ImFluentExpandDirection_Up ) )
            {
                ...
                RadioButton( "Low", &s_priority, 0 );
                ImGui::SameLine();
                RadioButton( "Normal", &s_priority, 1 );
                ImGui::SameLine();
        ...
        if ( BeginControlExample( "Expand / collapse events" ) )
        {
            ...
            if ( BeginExpander( "Notifications", &s_open, ImFluentExpandDirection_Down, &just_expanded, &just_collapsed ) )
            {
    ...
    static void Page_Item_TabView()
    {
        ...
        if ( BeginControlExample( "Tabs" ) )
        {
            if ( BeginTabView( "##tv" ) )
            {
                ...
                {
                    TextBlock( "Overview content", ImFluentTextStyle_Body );
                    EndTabItem();
                }
                if ( BeginTabItem( "Details" ) )
                {
                    TextBlock( "Details content", ImFluentTextStyle_Body );
                    EndTabItem();
                }
                if ( BeginTabItem( "Settings" ) )
                {
                    TextBlock( "Settings content", ImFluentTextStyle_Body );
                    EndTabItem();
                }
                EndTabView();
            }
        ...
        if ( BeginControlExample( "Closeable tabs + Add button (middle-click also closes)" ) )
        {
            static struct Tab
            {
                char title[ 32 ];
                bool open;
            } tabs[ 8 ]         = { { "Document 1", true }, { "Document 2", true }, { "Document 3", true } };
            static int next_doc = 4;
            if ( BeginTabView( "##tv-add" ) )
            {
                for ( int i = 0; i < 8; ++i )
                {
                    if ( !tabs[ i ].open || tabs[ i ].title[ 0 ] == 0 )
                        continue;
    ...
    static void Page_Item_NavigationView()
    {
        ...
        if ( BeginControlExample( "Side pane with title, back button, search and settings" ) )
        {
            ...
            NavPaneTitle( "My App" );
            NavPaneAutoSuggestBox( "##nav-search", search_buf, sizeof( search_buf ), search_items, IM_ARRAYSIZE( search_items ), NULL, "Search" );
            if ( NavBackButton( sel != 0, true ) )
            {
                back_clicks++;
                sel = 0;
            }
            if ( NavItem( "Home", sel == 0, ImFluentIcon_Home ) )
                sel = 0;
            if ( NavItem( "Inbox", sel == 1, ImFluentIcon_Mail ) )
                sel = 1;
            NavSubHeader( "More" );
            if ( NavItem( "Folders", sel == 2, ImFluentIcon_Folder ) )
                sel = 2;
            if ( NavItem( "Contacts", sel == 3, ImFluentIcon_Contact ) )
                sel = 3;
            if ( NavSettingsItem( sel == 4 ) )
                sel = 4;
            EndNavigationView();
            ImGui::SameLine();
            ImGui::BeginGroup();
    ...
    static void Page_Item_SelectorBar()
    {
        ...
        if ( BeginControlExample( "Pivot" ) )
        {
            ...
            {
                if ( SelectorBarItem( "Day", s == 0 ) )
                    s = 0;
                if ( SelectorBarItem( "Week", s == 1 ) )
                    s = 1;
                if ( SelectorBarItem( "Month", s == 2 ) )
                    s = 2;
                if ( SelectorBarItem( "Year", s == 3 ) )
                    s = 3;
                EndSelectorBar();
            }
            ControlExampleOutput( "Selected: %d", s );
    ...
    static void Page_Item_InfoBar()
    {
        ...
        if ( BeginControlExample( "Action button + close" ) )
        {
            ...
            if ( InfoBar( ImFluentInfoSeverity_Informational, "Update available", "A new version is ready to install. Restart to apply.", &open, NULL, true, "Install" ) )
                install_clicks++;
        ...
        {
            static bool open = true;
            InfoBar( ImFluentInfoSeverity_Success, "Saved", "Changes have been written to disk.", &open, NULL, false );
        }
        EndControlExample();
    }
    ...
    static void Page_Item_InfoBadge()
    {
        ...
        {
            TextBlock( "Dot:", ImFluentTextStyle_Body );
            ImGui::SameLine();
            InfoBadge();
            ImGui::Spacing();
            TextBlock( "Count 3:", ImFluentTextStyle_Body );
            ImGui::SameLine();
            InfoBadge( 3 );
            ImGui::Spacing();
            TextBlock( "Count 99+:", ImFluentTextStyle_Body );
            ImGui::SameLine();
            InfoBadge( 101 );
        }
        EndControlExample();
    ...
    static void Page_Item_MenuFlyout()
    {
        ...
        {
            if ( Button( "Open share menu" ) )
                OpenMenuFlyout( "##mf2" );
            if ( BeginMenuFlyout( "##mf2" ) )
            {
                SetNextItemGlyph( ImFluentIcon_Mail );
                MenuFlyoutItem( "Email a copy" );
                SetNextItemGlyph( ImFluentIcon_Print );
                MenuFlyoutItem( "Print" );
                SetNextItemGlyph( ImFluentIcon_Share );
                MenuFlyoutItem( "Share to…", "Ctrl+S" );
                MenuFlyoutSeparator();
                SetNextItemGlyph( ImFluentIcon_Folder );
                MenuFlyoutItem( "Save to folder…" );
                EndMenuFlyout();
            }
        }
        EndControlExample();
        ...
        if ( BeginControlExample( "Toggle, Radio, and cascading SubItem" ) )
        {
            ...
            ControlExampleOutput( "wrap=%s  ruler=%s  zoom=%d%%", word_wrap ? "on" : "off", show_ruler ? "on" : "off", zoom_pct );
        }
        EndControlExample();
    }
    ...
    static void Page_Item_AppBarButton()
    {
        ...
        {
            AppBarButton( "Add", ImFluentIcon_Add );
            ImGui::SameLine();
            AppBarButton( "Edit", ImFluentIcon_Edit );
            ImGui::SameLine();
            AppBarButton( "Delete", ImFluentIcon_Delete );
            ImGui::SameLine();
            AppBarSeparator();
            ImGui::SameLine();
        ...
        if ( BeginControlExample( "AppBarToggleButton" ) )
        {
            ...
            AppBarToggleButton( "Bold", "B", &bold );
            ImGui::SameLine();
            AppBarToggleButton( "Italic", "I", &italic );
            ImGui::SameLine();
        ...
        if ( BeginControlExample( "LabelPosition: Bottom (default), Right, Collapsed" ) )
        {
            ...
            AppBarButton( "Refresh", ImFluentIcon_Refresh );
            ImGui::SameLine();
    ...
    static void Page_Item_CalendarDatePicker()
    {
        ...
        if ( BeginControlExample( "Bounded date range" ) )
        {
            ...
            ImGui::PopItemWidth();
            ControlExampleOutput( "Date: %04d-%02d-%02d (range %04d-%02d-%02d ... %04d-%02d-%02d)", d.Year, d.Month, d.Day, min_d.Year, min_d.Month, min_d.Day, max_d.Year, max_d.Month, max_d.Day );
        }
        EndControlExample();
    }
    ...
    static void Page_Item_CommandBar()
    {
        ...
        if ( BeginControlExample( "Primary + overflow" ) )
        {
            ...
            {
                if ( AppBarButton( "Save", ImFluentIcon_Save ) )
                    format_clicks++;
                if ( AppBarButton( "Copy", ImFluentIcon_Copy ) )
                    format_clicks++;
                if ( AppBarButton( "Cut", ImFluentIcon_Cut ) )
                    format_clicks++;
                if ( AppBarButton( "Paste", ImFluentIcon_Paste ) )
                    format_clicks++;
                AppBarSeparator();
                if ( AppBarButton( "Share", ImFluentIcon_Share ) )
                    format_clicks++;
    ...
    static void Page_Item_SplitView()
    {
        ...
        if ( BeginControlExample( "Configurable" ) )
        {
            ...
            if ( BeginSplitView( "##sv1", &pane_open, ( ImFluentSplitViewDisplayMode )mode_pick, ( ImFluentSplitViewPanePlacement )place_pick ) )
            {
    ...
    static void Page_Item_WrapPanel()
    {
        ...
        {
            static const char * tags[] = { "Productivity", "Design", "Networking", "Storage", "Security", "Accessibility", "Performance", "Privacy", "Sync", "Themes" };
            BeginWrapPanel();
    ...
    static void Page_Item_TitleBar()
    {
        ...
        if ( BeginControlExample( "Back + pane toggle + icon + subtitle" ) )
        {
            ...
            {
                if ( TitleBarBackButton( back_clicks > 0 ? true : false ) )
                    --back_clicks;
                if ( TitleBarPaneToggleButton() )
                    ++toggle_clicks;
            ...
            if ( Button( "Push back-stack item" ) )
                ++back_clicks;
    ...
    static void Page_Item_ColorPicker()
    {
        ...
        if ( BeginControlExample( "Color swatches" ) )
        {
            ...
            {
                ImGui::PushID( i );
                if ( ColorButton( "##swatch", ImColor( swatches[ i ] ).Value, ImGuiColorEditFlags_NoBorder | ImGuiColorEditFlags_NoTooltip, ImVec2( FluentDpx( 28.f ), FluentDpx( 28.f ) ) ) )
                    picked = i;
                ImGui::PopID();
                ImGui::SameLine();
            }
    ...
    static void Page_AllControls()
    {
        ...
        for ( int i = 0; i < g_ControlsCount; ++i )
        {
            ...
            if ( !c.PageFn )
                continue;
            if ( filter[ 0 ] && !std::strstr( c.Title, filter ) )
                continue;
            if ( shown > 0 )
            {
            ...
            ImGui::PushID( i );
            ImGui::BeginChild( c.UniqueId, ImVec2( -FLT_MIN, 0 ), ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_NavFlattened, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse );
            c.PageFn();
            ImGui::EndChild();
            ImGui::PopID();
            ++shown;
        ...
        if ( shown == 0 )
            TextBlock( "(no matches)", ImFluentTextStyle_Caption );
    }
    ...
    static void Page_Section( const char * groupId )
    {
        ...
        for ( int i = 0; i < g_GroupsCount; ++i )
            if ( std::strcmp( g_Groups[ i ].GroupId, groupId ) == 0 )
            {
                gi = &g_Groups[ i ];
                break;
            }
        ...
        {
            const ControlInfo & c = g_Controls[ i ];
            if ( std::strcmp( c.GroupId, groupId ) != 0 )
                continue;
            ImGui::PushID( i );
    ...
    static void Page_Settings()
    {
        ...
        {
            if ( SelectorBarItem( "Light", cur == ImFluentThemePreset_Light ) )
                ImFluent::SetThemePreset( ImFluentThemePreset_Light );
            if ( SelectorBarItem( "Dark", cur == ImFluentThemePreset_Dark ) )
                ImFluent::SetThemePreset( ImFluentThemePreset_Dark );
            if ( SelectorBarItem( "High contrast", cur == ImFluentThemePreset_HighContrast ) )
                ImFluent::SetThemePreset( ImFluentThemePreset_HighContrast );
            EndSelectorBar();
        }
    ...
    static void RenderCurrentPage()
    {
        ...
        if ( std::strcmp( id, "Home" ) == 0 )
        {
            Page_Home();
            return;
        }
        if ( std::strcmp( id, "AllControls" ) == 0 )
        {
            Page_AllControls();
            return;
        }
        ...
        if ( std::strncmp( id, "Section_", 8 ) == 0 )
        {
            Page_Section( id + 8 );
            return;
        }
    ...
    static void DrawTitleBar( bool * p_open )
    {
        ...
        ImGui::BeginChild( "##titlebar", ImVec2( 0, h ), ImGuiChildFlags_None, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse );
        ...
        ImGui::SameLine();
        ImGui::Dummy( ImVec2( FluentDpx( 12.f ), 0 ) );
        ImGui::SameLine();
    ...
    static void DrawNavigationPane()
    {
        ...
        for ( int g = 0; g < g_GroupsCount; ++g )
        {
            ...
            if ( open )
            {
                ...
                {
                    const ControlInfo & c = g_Controls[ i ];
                    if ( std::strcmp( c.GroupId, gi.GroupId ) != 0 )
                        continue;
                    const char * ipid;
                    const char * ipid_end;
    ...
    static void ShowDemoWindowImpl( bool * p_open )
    {
        ...
        ImGui::Begin( "##imfluent-demo-root", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings );
        ImGui::PopStyleVar( 3 );
